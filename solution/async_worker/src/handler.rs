...
mod tests {
    ...
    // Test for persist_base_plans with valid data
    #[tokio::test]
    async fn test_persist_base_plans_valid() {
        init_env();
        let provider_id = Uuid::new_v4();
        let provider_name = "Test Provider".to_string();

        // Insert a provider row using add_or_update_provider
        {
            use storage::models::providers::NewProvider;
            use storage::provider::add_or_update_provider;
            let mut pg_pool = get_db_connection().await.expect("DB connection");
            let _ = add_or_update_provider(
                &mut pg_pool,
                NewProvider {
                    providers_id: provider_id,
            ...
            .expect("Failed to insert provider for test");
        }
